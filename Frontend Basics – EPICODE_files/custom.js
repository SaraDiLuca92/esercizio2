jQuery(document).ready(function ($) {
	/**************************** */
	// Metriche modal matteo ragusa
	/*************************** */
	var modal = document.getElementById('metricheModalCenter');

	// Get the button that opens the modal
	var btn = document.getElementById("metricheModalButton");
	...
	// checkbox nel modal 
	$('#metriche_set_colon input[type=checkbox]').change(function () {
		var $checkboxes = $('#metriche_set_colon input[type=checkbox]');
		if ($(this).is(":checked")) {
		...
		if (countCheckedCheckboxes > 10) {
			$('#conferma_metriche').prop('disabled', true);
		} else {
			$('#conferma_metriche').prop('disabled', false);
		}

	});

	// reset checkbox nel modal
	$('#resetta_metriche').click(function () {
	...
	//Menu active class
	$("ul#st-primary-menu .bp-menu a").click(function () {
		$("ul#st-primary-menu .bp-menu a").removeClass("active");
		$(this).addClass("active");
	});

	if ($_GET('mitem') == 'activity' || $_GET('amitem') == 'classfeed') {
		$('li#bp-gactivity a').addClass("active");
		$('ul.side-panel-menu li.lms_menus').addClass('current_page_item');
	} else if ($_GET('mitem') == 'allmembers' || $_GET('amitem') == 'clsmembers') {
		$('li#bp-gclass a').addClass("active");
		$('ul.side-panel-menu li.lms_menus').addClass('current_page_item');
	} else if ($_GET('gtype') == 'coach-type' || $_GET('amitem') == 'coachfeed') {
		$('li#bp-gcoach a').addClass("active");
		$('ul.side-panel-menu li.lms_menus').addClass('current_page_item');
	} else if ($_GET('amitem') == 'corsi') {
		$('li#bp-corsi a').addClass("active");
		$('ul.side-panel-menu li.lms_menus').addClass('current_page_item');
	}
	...
	$('ul#st-primary-menu .bp-menu a').each(function () {
		// and test its normalized href against the url pathname regexp
		if (urlRegExp.test(this.href.replace(/\/$/, ''))) {
			$(this).addClass('active');
			if (getLastPartOfUrl(url) == 'corsi' || getLastPartOfUrl(url) == 'forums') {
				$('ul.side-panel-menu li.lms_menus').addClass('current_page_item');
			}
			if (getLastPartOfUrl(url) == 'pagamenti-studenti' || getLastPartOfUrl(url) == 'crm-studenti') {
				$('ul.side-panel-menu li.crm_menus').addClass('current_page_item');
			}
			if (getLastPartOfUrl(url) == 'recruiting-studenti' || getLastPartOfUrl(url) == 'recruiting-classi') {
				$('ul.side-panel-menu li.rec_menus').addClass('current_page_item');
			}
			// aggiunta per metriche e kpi matteo ragusa
			if (getLastPartOfUrl(url) == 'metriche-studenti' || getLastPartOfUrl(url) == 'metriche-docenti' || getLastPartOfUrl(url) == 'metriche-aziende') {
				$('ul.side-panel-menu li.met_menus').addClass('current_page_item');
			}
		}
	});

	$('.profileusermenu').click(function (e) {
		e.preventDefault();
		$('.sub-menu-inner').slideToggle("400");
	});

	// trigger the wdmChkPwdStrength
	$('.acf-field.acf-field-password.pwd .acf-input .acf-input-wrap').after('<span id="password-strength"></span>');
	$('body').on('keyup', 'input[name="acf[field_6065b7debc068]"]', function (event) {
		wdmChkPwdStrength(
			// password field
			$('input[name="acf[field_602bcbd880ce8]"]'),
			$('input[name="acf[field_6065b7debc068]"]'),
			// strength status
			$('.acf-field.acf-field-password.pwd #password-strength'),
			// Submit button
			$('input[type=submit]'),
	...
	$('#termspdf input[name="acf[field_602e54a79e1d4][field_602e55b0e1210]"]').change(function (event) {
		event.preventDefault();
		if ($(this).is(':checked')) {
			$('#termspdf').find('.acf-label p a').css('pointer-events', 'visible');
			$('#termspdf').find('.acf-label p a').attr('download', '');
			//downloadURI($('#directdownload').attr('href'));
		} else {
			$('#termspdf').find('.acf-label p a').css('pointer-events', 'none');
			$('#termspdf').find('.acf-label p a').removeAttr('download');
		}
	});

	$('#clausespdf input[name="acf[field_602e54a79e1d4][field_602e55e6e1211]"]').change(function (event) {
		event.preventDefault();
		if ($(this).is(':checked')) {
			$('#clausespdf').find('.acf-label p a').css('pointer-events', 'visible');
			$('#clausespdf').find('.acf-label p a').attr('download', '');
			//downloadURI($('#directdownload2').attr('href'));
		} else {
			$('#clausespdf').find('.acf-label p a').css('pointer-events', 'none');
			$('#clausespdf').find('.acf-label p a').removeAttr('download');
		}
	});

	$('#privacypdf input[name="acf[field_602e54a79e1d4][field_602e560ce1212]"]').change(function (event) {
		event.preventDefault();
		if ($(this).is(':checked')) {
			$('#privacypdf').find('.acf-label p a').css('pointer-events', 'visible');
			$('#privacypdf').find('.acf-label p a').attr('download', '');
			//downloadURI($('#directdownload3').attr('href'));
		} else {
			$('#privacypdf').find('.acf-label p a').css('pointer-events', 'none');
			$('#privacypdf').find('.acf-label p a').removeAttr('download');
		}
	});
	...
	if (lang == 'it') {
		...
		$('input.acf-button').attr('disabled', 'disabled');
	
		/*$('#student_enrollment > input').keyup(function() {
	
			var empty = false;
			$('#student_enrollment > input').each(function() {
				if ($(this).val() == '') {
					empty = true;
				}
			});
			console.log('field all '.empty);
			if (empty) {
				$('input.acf-button').attr('disabled', 'disabled'); 
			} else {
				$('input.acf-button').removeAttr('disabled'); 
			}
		});*/


		$('#student_enrollment input').on( 'input', function() {
			var empty = false;  //&& $(this).attr('id') != 'acf-field_602cae9849578'
			var enable = false;
			$('#student_enrollment input').each(function() { 
				if ($(this).val() == '' && $(this).attr('id') != undefined && $(this).attr('id') != 'acf-field_60ac7e205bb9b-no' && $(this).attr('id') != 'acf-field_60ac7e205bb9b-si' && $(this).attr('id') != 'acf-_validate_email' && $(this).attr('id') != 'acf-field_6065a8a632dfc-input' && $(this).attr('id') != 'acf-field_602f96966b609' && $(this).attr('id') != 'acf-field_602f96d16b60a' && $(this).attr('id') != 'acf-field_6065a5fbc2816' ) {
					empty = true;
				}
			});
			...
			if (empty == true) {
				...
				if($('#acf-field_602e54a79e1d4-field_602e560ce1212').is(':checked') && ($('#acf-field_60ac7e205bb9b-si').is(':checked') || $('#acf-field_60ac7e205bb9b-no').is(':checked')) ){
					// check COdice fiscale 
					var nome = $('#acf-field_602bcb8080ce5').val();
					var cognome = $('#acf-field_602bcb8e80ce6').val();
					var localita = $('#acf-field_602bccff80ceb').val();
					var dataDiNascita = $('#acf-field_6034af443817b').val();
					var sesso = $('#acf-field_602bcc7e80cea').val();
					var codice_fiscale_input = $('#acf-field_604f3a2bddd58').val();

					if(nome != '' && cognome != '' && localita != '' && dataDiNascita.length > 7 && codice_fiscale_input.length >14){
						//var calcolo_codice_fiscale = Calcola_CF(nome,cognome,localita,dataDiNascita,sesso );
		
						//console.log('calcola codice fiscale '+calcolo_codice_fiscale);
		
						//if(calcolo_codice_fiscale == codice_fiscale_input){
							enable = true;
							$('.acf-field-604f3a2bddd58 #address_error_msg').remove();
							//console.log('codice valido');
							
						/*}else{
							enable = false;
							console.log('codice non valido');
							if( $('#address_error_msg').length === 0){
								$('#acf-field_604f3a2bddd58').parent().append('<label id="address_error_msg" class="error">Controlla il Codice fiscale</label>');
								}
						}*/
						if (enable == true){
							$('input.acf-button').removeAttr('disabled');
		...
		/* auto feel email field */
	
		var uemail = getUrlParameter('uemail');
	
		$('#acf-field_602bcbca80ce7').val(uemail);
		$('#acf-field_602bcbca80ce7').parent().parent().parent().find('label').addClass('float-lable-focus');
		$('#acf-field_602bcbca80ce7').attr('readonly',true)
	
	}else{ 
		$('#register_user').attr('disabled', 'disabled');
	
		/*$('#international_registration_w > div input').keyup(function() {
	
			var empty = false;
			$('#international_registration_w > div input').each(function() {
				var input_id = $(this).attr('id');
				if ($(this).val() == '' && input_id != 'hiddengrecaptcha') {
					empty = true;
				}
			});
		...
		$('#international_registration_w input').on( 'input', function() {
			var empty = false;//&& $(this).attr('id') != 'residence_city' && $(this).attr('id') != 'zip_code'
			$('#international_registration_w input').each(function() {
				if ($(this).val() == '' && $(this).attr('id') != undefined && $(this).attr('id') != 'hiddengrecaptcha') {
					empty = true;
				}
			});
...
function wdmChkPwdStrength($pwd, $confirmPwd, $strengthStatus, $submitBtn, blacklistedWords) {
	...
	$strengthStatus.removeClass('short bad good strongs');
	...
	switch (pwdStrength) {
		...
		case 4:
			$strengthStatus.addClass('strongs').html(pwsL10n.strong);
			break;
...
function downloadURI(uri) {
	...
	link.setAttribute('download', uri.split('/').pop());
	link.href = uri;
...
//START Codice Fiscale
function Calcola_CF(nome_input,cognome_input,localita_input,dataDiNascita,sesso ) {
  
  var localita = localita_input.substr(0, localita_input.indexOf(','));
	...
	///####################INSERIMENTO DINAMICO DALLA FORM##############################///
	 ...
	 var nome = nome_input.replace(/\s/g, '');
	 var cognome = cognome_input.replace(/\s/g, '');
	 ...
	 var custom_data = dataDiNascita.substring(0, 4) + '-' +dataDiNascita.substring(4, 6) + '-' + dataDiNascita.substring(6);
	 var from = custom_data.split("-")
	 var data = new Date(from[0], from[1] - 1, from[2]);
